...
# def collocate(xd,xa,u,p,nk,d,dynamics, out_fun,out):                          #NOTE: NEEDED FOR MAIN.PY MODEL VALIDATION # TODO: adjust model validation code ...
def collocate(xd,xa,u,p,puni,l,nk,d,dynamics, out_fun,out):
    ...
    V = ca.struct_symMX([
                      (
                       ...
                       ca.entry('tf'),
                       ca.entry('vlift'),
                       ca.entry('l',struct = l)      # tether length             #NOTE: COMMENT FOR MAIN.PY MODEL VALIDATION # TODO: adjust model validation code ... 
    ...
    P = ca.struct_symMX([
                      ...
                      ca.entry('puni', struct = puni),                          #NOTE: COMMENT FOR MAIN.PY MODEL VALIDATION # TODO: adjust model validation code ...
    ...
    for k in range(nk):
        ...
        for j in range(1,d+1):
            ...
            fk = dynamics(V['Xd',k,j],xp_jk/h/V['tf'], V['XA',k,j-1], V['U',k], P['p',k,j],P['puni'], V['l'])
            coll_cstr.append(fk)
        ...
        for j in range(1,d+1):
            ...
            outk = out_fun(V['Xd',k,j],V['XA',k,j-1], P['p',k,j],P['puni'],V['l'])
